...
SELECT 
    ...
    e.idExpedicao,
    SUM(CAST(REPLACE(s.valor, ',', '.') AS DECIMAL(10,2)) * s.qtd) + MIN(CAST(REPLACE(s.frete, ',', '.') AS DECIMAL(10,2)))
...
ORDER BY 
    SUM(CAST(REPLACE(s.valor, ',', '.') AS DECIMAL(10,2)) * s.qtd) + MIN(CAST(REPLACE(s.frete, ',', '.') AS DECIMAL(10,2))) DESC,
    SUM(s.qtd) DESC;